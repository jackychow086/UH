...
<head>
    ...
    <title>Impersonator | 氫能航空動力專家</title>
...
<body>
    ...
    <nav class="bg-white/80 backdrop-blur-md border-b sticky top-0 z-50 px-8 py-4">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            <span class="text-xl font-bold tracking-tight text-slate-800">Impersonator <span class="text-blue-600">Aviation</span></span>
    ...
    <header id="vision" class="hero-bg text-white py-24 px-6 text-center">
        <div class="max-w-4xl mx-auto">
            ...
            <p class="text-xl text-blue-100 mb-12 font-light">
                Impersonator 致力於為 eVTOL 提供 LH2 儲存與燃料電池動力系統 [cite: 3, 77]。
            </p>
            <div class="flex flex-wrap justify-center gap-6">
                <div class="bg-white/10 px-6 py-3 rounded-full border border-white/20">TRL 6-7 技術驗證 [cite: 12, 78]</div>
                <div class="bg-white/10 px-6 py-3 rounded-full border border-white/20">專注高增長市場 [cite: 3, 79]</div>
            </div>
        </div>
    </header>
    ...
    <section class="py-20 px-6 max-w-4xl mx-auto text-center reveal">
        ...
        <p class="text-lg text-slate-600 leading-relaxed">
            對於實際航程需求，電池過重，氣態氫體積則過於龐大 [cite: 7, 81]。<br>
            對於 eVTOL，僅需 <span class="highlight">數十公斤液態氫 (LH2)</span> 即可達成所需的續航時間 [cite: 8, 81]。
        </p>
    </section>
    ...
    <section id="tech" class="py-16 px-6 max-w-7xl mx-auto grid md:grid-cols-3 gap-8">
        <div class="data-card p-8 reveal">
            ...
            <p class="text-slate-500 text-sm mb-6">成功飛行測試 <span class="highlight">1MW 氫電動力系統</span> (Dash 8-300 平台) [cite: 15, 17, 87]。</p>
            <button onclick="toggleInfo('tech1')" class="text-blue-600 text-sm font-bold hover:underline">查看詳情</button>
            <div id="tech1" class="hidden mt-4 text-xs bg-slate-50 p-4 rounded-lg">包含模塊化 LH2 儲存與真空絕緣技術 [cite: 18]。</div>
        </div>

        <div class="data-card p-8 reveal">
            <div class="text-emerald-600 text-3xl mb-6"><i class="fas fa-chart-line"></i></div>
            <h3 class="text-xl font-bold mb-4">2. 鎖定領先市場</h3>
            <p class="text-slate-500 text-sm mb-6">中國是全球領先市場，已有億航獲得營運證書 [cite: 52, 90]。</p>
            <button onclick="toggleInfo('tech2')" class="text-blue-600 text-sm font-bold hover:underline">市場數據</button>
            <div id="tech2" class="hidden mt-4 text-xs bg-slate-50 p-4 rounded-lg">2035 年全球規模預計突破 <span class="highlight">400 億美元</span> [cite: 53]。</div>
        </div>

        <div class="data-card p-8 reveal">
            <div class="text-orange-600 text-3xl mb-6"><i class="fas fa-boxes"></i></div>
            <h3 class="text-xl font-bold mb-4">3. 商業模式</h3>
            <p class="text-slate-500 text-sm mb-6">銷售 <span class="highlight">Turnkey 系統套件</span>，含儲存模組與動力系統 [cite: 38, 93]。</p>
            <button onclick="toggleInfo('tech3')" class="text-blue-600 text-sm font-bold hover:underline">執行計畫</button>
            <div id="tech3" class="hidden mt-4 text-xs bg-slate-50 p-4 rounded-lg">Phase 1: 圖盧茲核心團隊 [cite: 29]；Phase 2: 中國供應鏈量產 [cite: 33, 35]。</div>
        </div>
    </section>
    ...
    <div id="modal" class="fixed inset-0 bg-slate-900/80 hidden items-center justify-center z-[100] p-4 backdrop-blur-sm">
        <div class="bg-white p-8 rounded-3xl max-w-lg w-full text-center border border-blue-500">
            <h4 class="text-2xl font-bold mb-4 text-blue-600">Impersonator 核心策略</h4>
            <p class="text-slate-600 mb-8 text-left text-sm leading-relaxed">
                1. 避開漫長認證，優先對接急需航程方案的中國 eVTOL 廠商 [cite: 46, 99]。<br>
                2. 利用中國成熟的 LH2 低溫製造能力進行量產 [cite: 36]。<br>
                3. 以已驗證的 TRL 6-7 技術作為競爭護城河 [cite: 12, 78]。
            </p>
